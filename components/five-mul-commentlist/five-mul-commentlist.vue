			...
			<view class="view-comment-list-item">
				<view class="comment-title">
					...
					<image
						...
						mode="aspectFill"
					></image>
					<text class="praise-num" style="margin-left: 10rpx;">0</text>
				</view>
				<text class="comment-content">{{ item.content }}</text>
				<view class="comment-sub">
					<text class="comment-sub-time">{{ item.createTime|formatDate }}</text>
					<view class="comment-sub-recomment" style="color: #999999;" v-if="item.deleteEnable&&false" @click="clickDelete(item)">删除</view>
					<view class="comment-sub-recomment" @click="clickRecomment(item)">回复</view>
				</view>
				<view class="comment-child" v-show="item.children.length > 0">
					<view class="comment-child-item" v-for="(itemChild, indexChild) in item.children" :key="indexChild">
						<text class="comment-child-item-name" @click="clickRecommentChild(itemChild)">{{ itemChild.userAccount + '：' }}</text>
						<!-- <text class="comment-child-item-content">{{ (itemChild.SECONDNICKNAME ? '回复 ' + itemChild.SECONDNICKNAME + '：' : '') + itemChild.COMMENT }}</text> -->
						<text class="comment-child-item-content" v-if="itemChild.replyAccount">回复</text>
						<text class="comment-child-item-content">{{ itemChild.replyAccount ? itemChild.replyAccount : '' }}</text>
						<text class="comment-child-item-content" v-if="itemChild.replyAccount">：</text>
						<text class="comment-child-item-content">{{ itemChild.content }}</text>
						<text v-if="itemChild.deleteEnable&&false" class="comment-child-item-delete" @click="clickDeleteChild(itemChild)">删除</text>
					</view>
				</view>
			</view>
		</view>
