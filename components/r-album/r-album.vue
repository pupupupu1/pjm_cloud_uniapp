<template>
	<view class="dragsort-box">
		<movable-area id="drag" class="dragsort-area">
			<view id="dragsort-before" class="dragsort-before"><slot name="before"></slot></view>
			...
			<movable-view
				...
				class="dragsort-view"
				:class="{ 'dragsort-view-active': isDrag && activeModel && activeModel.id == item.id }"
				:style="{
					width: item.width + 'px',
			...
			>
				...
				<view v-if="item.isAdd" class="dragsort-view-item" @click.stop="onAdd">
					<view class="btnAdd"><u-icon class="btnAdd-icon" name="plus" color="#B2B2B2" size="90"></u-icon></view>
				</view>
			</movable-view>
			<view id="dragsort-after" class="dragsort-after"><slot name="after"></slot></view>
	...
	methods: {
		...
		touchmove(e) {
			...
			touchMoveTimeOut = setTimeout(function() {
				...
				if (lastTouchE) {
					if (Math.abs(e.mp.touches[0].clientY - lastTouchE.mp.touches[0].clientY) < 20 && Math.abs(e.mp.touches[0].clientX - lastTouchE.mp.touches[0].clientX) < 20) {
						return;
					}
				}
				lastTouchE = e;
