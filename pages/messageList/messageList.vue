			...
			<u-cell-group v-for="item in msgList">
				<u-cell-item @tap="cellClick(item.optionId, item.isGroupMsg)" :arrow="false" :use-label-slot="true" hover-class="cell-hover-class">
					...
					<view slot="right-icon" v-if="item.waitReadMsgNum > 0 || true">
						<template>
							<view v-show="item.msgTime">{{ item.msgTime | formatDate }}</view>
...
export default {
	...
	computed: {
		...mapState(['userName', 'isOnLine', 'msgList', 'friendList', 'groupList'])
	},
	created() {
		var that = this;
		...
		uni.getStorage({
			...
			msglist.forEach(item => {
				let friendInfo = this.friendList.find(item2 => item2.user.userAccount == item.optionId);
				let groupInfo = this.groupList.find(item2 => item2.id == item.optionId);

				if (friendInfo) {
					item.headerUrl = friendInfo.user.userHeader;
				} else if (groupInfo) {
					item.headerUrl = groupInfo.userGroupAvatarPath;
				}
			});
			//加载
			that.setMsgList(msglist);
