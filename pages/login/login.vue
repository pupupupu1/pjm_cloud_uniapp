<template>
	<view class="login">
		<view class="content">
			...
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="account" type="text" maxlength="11" placeholder="用户名" :focus="isFocus"></wInput>
				<wInput v-model="password" type="password" maxlength="11" placeholder="密码"></wInput>
			</view>
			<wButton class="wbutton" text="登 录" :rotate="isRotate" @tap="bindLogin1"></wButton>
...
export default {
	...
	computed: mapState(['needStopVedioTalk', 'vedioTalkPool', 'userName', 'forcedLogin', 'socketTask', 'msgList', 'talkSession', 'friendList', 'groupList']),
	mounted() {
		_this = this;
		//this.isLogin();
	},
	onLoad() {
		//请求loginrequest接口发起请求获取公钥
		// let token = uni.getStorageSync('userInfo');
		// if (!(token == undefined || token == '' || token == null)) {
		// 	commonUtil.reLaunch('../messageList/messageList');
		// }
	},
	methods: {
		...mapMutations(['bindLogin','setNeedStopVedioTalk', 'setVedioTalkPool', 'setTalkConfirm', 'login', 'setSocket', 'setMsgList', 'setFriendList', 'setGroupList', 'setTalkAccessToken']),
		isLogin() {},
		bindLogin1(){
		...
		// 		let friendInfo = this.friendList.find(item => item.user.userAccount == optionname);
		...
		// 			msgbody.header.talkAccessToken = that.accessToken;
		// 			msgbody.action = '5';
		...
		// 					console.log(that.needStopVedioTalk);
		...
		// 					console.log(item);
		// 					this.dealMsg(JSON.parse(item), userInfo);
		// 				});
		// 			}
		// 		});
			
		// 		this.connWebsocket(userInfo)
		// 		// return SocketTask;
